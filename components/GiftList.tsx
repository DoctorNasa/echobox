...
export function GiftList({ userAddress }: GiftListProps) {
  ...
  const { sentGifts, receivedGifts } = useMemo(() => {
    const sent = mockGifts.filter(gift => 
      gift.sender.toLowerCase() === userAddress.toLowerCase()
    );
    const received = mockGifts.filter(gift => 
      gift.recipient.toLowerCase() === userAddress.toLowerCase()
    );
  ...
  const giftCounts = useMemo(() => {
    ...
    return {
      ...
      pending: allUserGifts.filter(g => g.status === GiftStatus.PENDING).length,
      claimable: receivedGifts.filter(g => g.status === GiftStatus.CLAIMABLE).length,
      claimed: allUserGifts.filter(g => g.status === GiftStatus.CLAIMED).length
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl text-yellow-600 mb-1">{giftCounts.pending}</div>
            <div className="text-sm text-muted-foreground">Pending</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl text-green-600 mb-1">{giftCounts.claimable}</div>
            <div className="text-sm text-muted-foreground">Claimable</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl text-gray-600 mb-1">{giftCounts.claimed}</div>
